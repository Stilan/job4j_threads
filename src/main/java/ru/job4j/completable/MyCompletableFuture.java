 ...
 * Данный класс имеет много методов.
 * Мы рассмотрим только некоторые из них. Объяснение будет идти на бытовых примерах.
 */
public class MyCompletableFuture {

    /**
     * Пример runAsync() возвращает CompletableFuture<Void>
    ...
    /**
     * supplyAsync() CompletableFuture<T>, второй
     * @param product
     * @return
     */
    ...
    /**
     * Пример thenRun()  позволяет выполнить задачу типа Runnable после выполнения асинхронной задачи
     * @throws Exception
     */
    ...
    /**
     * Пример thenAccept() В отличие от thenRun(), этот метод имеет доступ к результату CompletableFuture.
     * @throws Exception
     */
    ...
    /**
     * Пример thenApply() Также имеет доступ к результату.
     * Как раз благодаря этому, мы можем произвести преобразование полученного результата.
     * @throws Exception
     */
    public static void thenApplyExample2() throws Exception {
        CompletableFuture<String> bm = buyProduct("Молоко")
